...
class TestGlueJob(unittest.TestCase):
    ...
    def setUpClass(cls):
        # setup moto server
        cls.process = subprocess.Popen("moto_server s3", stdout=subprocess.PIPE,
                                       shell=True, preexec_fn=os.setsid)

        # create s3 connection, bucket and s3 url's
        cls.s3_conn = boto3.resource('s3', region_name='eu-central-1', endpoint_url=TestGlueJob.S3_MOCK_ENDPOINT)
        bucket = 'bucket'
